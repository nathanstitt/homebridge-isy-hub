...
class ISYPlatform {
  constructor(log, config) {
    ...
    this.debugLoggingEnabled =
      config.debugLoggingEnabled == undefined
        ? false
        : config.debugLoggingEnabled;
    this.includeAllScenes =
      config.includeAllScenes == undefined ? false : config.includeAllScenes;
    this.includedScenes =
      config.includedScenes == undefined ? [] : config.includedScenes;
    this.isy = new ISY(
      this.host,
      this.username,
      this.password,
  ...
  logger(msg) {
    if (
      this.debugLoggingEnabled ||
      (process.env.ISYJSDEBUG != undefined && process.env.IYJSDEBUG != null)
    ) {
      var timeStamp = new Date();
      this.log(
        `${timeStamp.getYear()}-${timeStamp.getMonth()}-${timeStamp.getDay()}#${timeStamp.getHours()}:${timeStamp.getMinutes()}:${timeStamp.getSeconds()}- ${msg}`
      );
    }
  }
  ...
  shouldIgnore(device) {
    ...
    if (
      device.deviceType == this.isy.DEVICE_TYPE_SCENE &&
      this.includeAllScenes == false
    ) {
      ...
      for (var rule of this.config.ignoreDevices) {
        ...
        if (rule.lastAddressDigit != undefined && rule.lastAddressDigit != "") {
          if (
            deviceAddress.indexOf(
              rule.lastAddressDigit,
              deviceAddress.length - 2
            ) == -1
          ) {
            continue;
          }
        ...
        this.logger(
          `ISYPLATFORM: Ignoring device: ${deviceName} [${deviceAddress}] because of rule [${rule.nameContains}] [${rule.lastAddressDigit}] [${rule.address}]`
        );
        return true;
      }
    }
  ...
  renameDeviceIfNeeded(device) {
    ...
    for (var rule of this.config.renameDevices) {
      ...
      if (rule.newName == undefined) {
        this.logger(
          `ISYPLATFORM: Rule to rename device is present but no new name specified. Impacting device: ${deviceName}`
        );
        return deviceName;
      } else {
        this.logger(
          `ISYPLATFORM: Renaming device: ${deviceName}[${deviceAddress}] to [${rule.newName}] because of rule [${rule.nameContains}] [${rule.lastAddressDigit}] [${rule.address}]`
        );
        return rule.newName;
      }
    }
  ...
  accessories(callback) {
    ...
    this.isy.initialize(() => {
      ...
      for (var index = 0; index < deviceList.length; index++) {
        ...
        if (!that.shouldIgnore(device)) {
          if (results.length >= 100) {
            that.logger(
              "ISYPLATFORM: Skipping any further devices as 100 limit has been reached"
            );
            break;
          }
          device.name = that.renameDeviceIfNeeded(device);
          if (garageInfo != null) {
            let relayAddress = device.address.substr(
              0,
              device.address.length - 1
            );
            relayAddress += "2";
            var relayDevice = that.isy.getDevice(relayAddress);
            homeKitDevice = new ISYGarageDoorAccessory(
              that.logger.bind(that),
              device,
              relayDevice,
              garageInfo.name,
              garageInfo.timeToOpen,
              garageInfo.alternate
            );
          }
      ...
      if (that.isy.elkEnabled) {
        if (results.length >= 100) {
          that.logger(
            "ISYPLATFORM: Skipping adding Elk Alarm panel as device count already at maximum"
          );
        } else {
          var panelDevice = that.isy.getElkAlarmPanel();
          panelDevice.name = that.renameDeviceIfNeeded(panelDevice);
          var panelDeviceHK = new ISYElkAlarmPanelAccessory(
            that.log,
            panelDevice
          );
      ...
      that.logger(
        `ISYPLATFORM: Filtered device has: ${results.length} devices`
      );
      callback(results);
    });
  }
  ...
  createAccessory(device) {
    ...
    if (device instanceof ISYLightDevice) {
      ...
      device.deviceType == that.isy.DEVICE_TYPE_ALARM_DOOR_WINDOW_SENSOR
    ) {
      return new ISYDoorWindowSensorAccessory(that.logger.bind(that), device);
    ...
    informationService
      ...
      .setCharacteristic(
        ...
        this.device.productName == null
          ? this.device.deviceFriendlyName
...
class ISYThermostatAccessory extends ISYBaseAccessory {
  ...
  getCoolSetPoint(callback) {
    this.log(
      `THERM: ${this.device.name} Getting Cooling Set Point - Device says: ${this.device.getCoolSetPoint()} translation says: ${this.toCelsius(this.device.getCoolSetPoint())}`
    );
    callback(null, this.toCelsius(this.device.getCoolSetPoint()));
  }
  getHeatSetPoint(callback) {
    this.log(
      `THERM: ${this.device.name} Getting Heating Set Point - Device says: ${this.device.getHeatSetPoint()} translation says: ${this.toCelsius(this.device.getHeatSetPoint())}`
    );
    callback(null, this.toCelsius(this.device.getHeatSetPoint()));
  }
  getHeatingCoolingMode(callback) {
    this.log(
      `THERM: ${this.device.name} Getting Heating Cooling Mode - Device says: ${this.device.getHeatingCoolingMode()}`
    );
    callback(null, this.device.getHeatingCoolingMode());
  }
  getHeatingCoolingState(callback) {
    this.log(
      `THERM: ${this.device.name} Getting Heating Cooling State - Device says: ${this.device.getHeatingCoolingState()}`
    );
    callback(null, this.device.getHeatingCoolingState());
  }
  getFanState(callback) {
    this.log(
      `THERM: ${this.device.name} Getting Fan State - Device says: ${this.device.getFanState()}`
    );
    callback(null, this.device.getFanState());
  }
  getHumidity(callback) {
    this.log(
      `THERM: ${this.device.name} Getting Current Rel. Humidity - Device says: ${this.device.getHumidity()}`
    );
  ...
  handleExternalChange() {
    this.log(
      `THERM: ${this.device.name} Incoming external change. Device says: ${this.device.getCurrentTemperatureState()}`
    );
    this.thermostatService.updateCharacteristic(
      Characteristic.CurrentTemperature,
      this.toCelsius(this.device.getCurrentTemperatureState())
    );
    this.thermostatService.updateCharacteristic(
      Characteristic.CoolingThresholdTemperature,
      this.toCelsius(this.device.getCoolSetPoint())
    );
    this.thermostatService.updateCharacteristic(
      Characteristic.HeatingThresholdTemperature,
      this.toCelsius(this.device.getHeatSetPoint())
    );
    this.thermostatService.updateCharacteristic(
      Characteristic.TargetHeatingCoolingState,
      this.device.getHeatingCoolingMode()
    );
    this.thermostatService.updateCharacteristic(
      Characteristic.CurrentHeatingCoolingState,
      this.device.getHeatingCoolingState()
    );
    this.thermostatService.updateCharacteristic(
      Characteristic.CurrentFanState,
      this.device.getFanState()
    );
    this.thermostatService.updateCharacteristic(
      Characteristic.CurrentRelativeHumidity,
      this.device.getHumidity()
    );
  }
  ...
  getServices() {
    ...
    thermostatService
      ...
      .on("get", this.getHeatingCoolingMode.bind(this));
    thermostatService
  ...
  setCoolSetPoint(temp, callback) {
    this.log(
      `THERM: ${this.device.name} Sending command to set cool set point (pre-translate) to: ${temp}`
    );
    var newSetPoint = this.toFahrenheit(temp);
    this.log(
      `THERM: ${this.device.name} Sending command to set cool set point to: ${newSetPoint}`
    );
    if (Math.abs(newSetPoint - this.device.getCoolSetPoint()) >= 1) {
      ...
      this.log(
        `THERM: ${this.device.name} command does not change actual set point`
      );
      callback();
    }
  }
  ...
  setHeatSetPoint(temp, callback) {
    this.log(
      `THERM: ${this.device.name} Sending command to set heat set point (pre-translate) to: ${temp}`
    );
    var newSetPoint = this.toFahrenheit(temp);
    this.log(
      `THERM: ${this.device.name} Sending command to set heat set point to: ${newSetPoint}`
    );
  ...
  setHeatingCoolingMode(mode, callback) {
    this.log(
      `THERM: ${this.device.name} Sending command to set heating/cooling mode (pre-translate) to: ${mode}`
    );
...
class ISYFanAccessory extends ISYBaseAccessory {
  ...
  translateFanSpeedToHK(fanSpeed) {
    switch (fanSpeed) {
      ...
      default:
        this.log(
          `FAN: ${this.device.name} !!!! ERROR: Unknown fan speed: ${fanSpeed}`
        );
        return 0;
    }
  }
  ...
  getFanRotationSpeed(callback) {
    this.log(
      `FAN: ${this.device.name} Getting fan rotation speed. Device says: ${this.device.getCurrentFanState()} translation says: ${this.translateFanSpeedToHK(this.device.getCurrentFanState())}`
    );
    callback(
      null,
      this.translateFanSpeedToHK(this.device.getCurrentFanState())
    );
  }
  // Sets the current state of the fan from the 0-100 level of HK to the isy-js level.
  setFanRotationSpeed(fanStateHK, callback) {
    this.log(
      `FAN: ${this.device.name} Sending command to set fan state (pre-translate) to: ${fanStateHK}`
    );
    var newFanState = this.translateHKToFanSpeed(fanStateHK);
    this.log(
      `FAN: ${this.device.name} Sending command to set fan state to: ${newFanState}`
    );
    if (newFanState != this.device.getCurrentFanState()) {
      ...
      this.log(
        `FAN: ${this.device.name} Fan command does not change actual speed`
      );
      callback();
    }
  }
  ...
  getIsFanOn() {
    this.log(
      `FAN: ${this.device.name} Getting fan is on. Device says: ${this.device.getCurrentFanState()} Code says: ${this.device.getCurrentFanState() != "Off"}`
    );
  ...
  setFanOnState(onState, callback) {
    this.log(
      `FAN: ${this.device.name} Setting fan on state to: ${onState} Device says: ${this.device.getCurrentFanState()}`
    );
    if (onState != this.getIsFanOn()) {
      if (onState) {
        this.log(`FAN: ${this.device.name} Setting fan speed to medium`);
        this.setFanRotationSpeed(
          this.translateFanSpeedToHK(this.device.FAN_LEVEL_MEDIUM),
          callback
        );
      } else {
        this.log(`FAN: ${this.device.name} Setting fan speed to off`);
        this.setFanRotationSpeed(
          this.translateFanSpeedToHK(this.device.FAN_OFF),
          callback
        );
      }
    } else {
      this.log(
        `FAN: ${this.device.name} Fan command does not change actual state`
      );
      callback();
    }
  }
  ...
  setPowerState(powerOn, callback) {
    this.log(
      `LIGHT: ${this.device.name} Setting powerstate to ${powerOn}`
    );
    if (powerOn != this.device.getCurrentLightState()) {
      this.log(
        `LIGHT: ${this.device.name} Changing powerstate to ${powerOn}`
      );
      ...
      this.log(
        `LIGHT: ${this.device.name} Ignoring redundant setPowerState`
      );
      callback();
    }
  }
  ...
  setBrightness(level, callback) {
    ...
    if (level != this.device.getCurrentLightDimState()) {
      if (level == 0) {
        this.log(
          `LIGHT: ${this.device.name} Brightness set to 0, sending off command`
        );
        ...
        this.log(
          `LIGHT: ${this.device.name} Changing Brightness to ${level}`
        );
      ...
      this.log(
        `LIGHT: ${this.device.name} Ignoring redundant setBrightness`
      );
      callback();
    }
  }
  ...
  handleExternalChange() {
    this.log(
      `FAN: ${this.device.name} Incoming external change. Device says: ${this.device.getCurrentFanState()}`
    );
...
class ISYOutletAccessory extends ISYBaseAccessory {
  ...
  setOutletState(outletState, callback) {
    this.log(
      `OUTLET: ${this.device.name} Sending command to set outlet state to: ${outletState}`
    );
  ...
  handleExternalChange() {
    this.outletService.updateCharacteristic(
      Characteristic.On,
      this.device.getCurrentOutletState()
    );
  }
...
class ISYLockAccessory extends ISYBaseAccessory {
  ...
  setTargetLockState(lockState, callback) {
    this.log(
      this,
      `LOCK: ${this.device.name} Sending command to set lock state to: ${lockState}`
    );
  ...
  handleExternalChange() {
    this.lockService.updateCharacteristic(
      ...
      this.getDeviceCurrentStateAsHK()
    );
    this.lockService.updateCharacteristic(
...
class ISYLightAccessory extends ISYBaseAccessory {
  ...
  handleExternalChange() {
    this.log(
      `LIGHT: ${this.device.name} Handling external change for light`
    );
    this.lightService.updateCharacteristic(
      Characteristic.On,
      this.device.getCurrentLightState()
    );
    if (this.dimmable) {
      this.lightService.updateCharacteristic(
        Characteristic.Brightness,
        this.device.getCurrentLightDimState()
      );
    }
  }
  // Handles request to get the current on state
...
class ISYSceneAccessory extends ISYBaseAccessory {
  ...
  setPowerState(powerOn, callback) {
    this.log(
      `SCENE: ${this.device.name} Setting powerstate to ${powerOn}`
    );
    if (!this.device.getAreAllLightsInSpecifiedState(powerOn)) {
      this.log(
        `SCENE: ${this.device.name} Changing powerstate to ${powerOn}`
      );
      this.device.sendLightCommand(powerOn, function(result) {
        callback();
      });
      ...
      this.log(
        `SCENE: ${this.device.name} Ignoring redundant setPowerState`
      );
      callback();
    }
  }
  ...
  handleExternalChange() {
    this.log(
      `SCENE: ${this.device.name} Handling external change for light`
    );
    if (
      this.device.getAreAllLightsInSpecifiedState(true) ||
      this.device.getAreAllLightsInSpecifiedState(false)
    ) {
      this.lightService.updateCharacteristic(
        Characteristic.On,
        this.device.getAreAllLightsInSpecifiedState(true)
...
class ISYDoorWindowSensorAccessory extends ISYBaseAccessory {
  ...
  handleExternalChange() {
    this.sensorService.setCharacteristic(
      ...
      this.translateCurrentDoorWindowState()
    );
  }
  // Returns the set of services supported by this object.
  getServices() {
...
class ISYMotionSensorAccessory extends ISYBaseAccessory {
  ...
  handleExternalChange() {
    this.sensorService.setCharacteristic(
      ...
      this.device.getCurrentMotionSensorState()
    );
  }
  // Returns the set of services supported by this object.
  getServices() {
...
class ISYElkAlarmPanelAccessory extends ISYBaseAccessory {
  ...
  setAlarmTargetState(targetStateHK, callback) {
    this.log(
      "ALARMSYSTEM: " +
        this.device.name +
        "Sending command to set alarm panel state to: " +
        targetStateHK
    );
    ...
    this.log(
      "ALARMSYSTEM: " +
        this.device.name +
        " Would send the target state of: " +
        targetState
    );
    if (this.device.getAlarmMode() != targetState) {
      ...
      this.log(
        "ALARMSYSTEM: " +
          this.device.name +
          " Redundant command, already in that state."
      );
      callback();
    }
  }
  ...
  translateAlarmCurrentStateToHK() {
    ...
    if (tripState >= this.device.ALARM_TRIP_STATE_TRIPPED) {
      ...
      if (
        sourceAlarmMode == this.device.ALARM_MODE_STAY ||
        sourceAlarmMode == this.device.ALARM_MODE_STAY_INSTANT
      ) {
        return Characteristic.SecuritySystemCurrentState.STAY_ARM;
      } else if (
        sourceAlarmMode == this.device.ALARM_MODE_AWAY ||
        sourceAlarmMode == this.device.ALARM_MODE_VACATION
      ) {
        return Characteristic.SecuritySystemCurrentState.AWAY_ARM;
      } else if (
        sourceAlarmMode == this.device.ALARM_MODE_NIGHT ||
        sourceAlarmMode == this.device.ALARM_MODE_NIGHT_INSTANT
      ) {
        ...
        this.log(
          "ALARMSYSTEM: " +
            this.device.name +
            " Setting to disarmed because sourceAlarmMode is " +
            sourceAlarmMode
        );
        return Characteristic.SecuritySystemCurrentState.DISARMED;
      }
  ...
  translateAlarmTargetStateToHK() {
    ...
    if (
      sourceAlarmState == this.device.ALARM_MODE_STAY ||
      sourceAlarmState == this.device.ALARM_MODE_STAY_INSTANT
    ) {
      return Characteristic.SecuritySystemTargetState.STAY_ARM;
    } else if (
      sourceAlarmState == this.device.ALARM_MODE_AWAY ||
      sourceAlarmState == this.device.ALARM_MODE_VACATION
    ) {
      return Characteristic.SecuritySystemTargetState.AWAY_ARM;
    } else if (
      sourceAlarmState == this.device.ALARM_MODE_NIGHT ||
      sourceAlarmState == this.device.ALARM_MODE_NIGHT_INSTANT
    ) {
  ...
  handleExternalChange() {
    this.log(
      "ALARMPANEL: " +
        this.device.name +
        " Source device. Currenty state locally -" +
        this.device.getAlarmStatusAsText()
    );
    this.log(
      "ALARMPANEL: " +
        this.device.name +
...
class ISYGarageDoorAccessory extends ISYBaseAccessory {
  constructor(log, sensorDevice, relayDevice, name, timeToOpen, alternate) {
    ...
    if (this.getSensorState()) {
      this.log(
        `GARAGE: ${this.name} Initial set during startup the sensor is open so defaulting states to open`
      );
      this.targetGarageState = Characteristic.TargetDoorState.OPEN;
      this.currentGarageState = Characteristic.CurrentDoorState.OPEN;
    } else {
      this.log(
        `GARAGE: ${this.name} Initial set during startup the sensor is closed so defaulting states to closed`
      );
  ...
  setTargetDoorState(targetState, callback) {
    ...
    if (this.currentGarageState == Characteristic.CurrentDoorState.OPEN) {
      if (targetState == Characteristic.TargetDoorState.CLOSED) {
        this.log(
          `GARAGE: ${this.device.name} Current state is open and target is closed. Changing state to closing and sending command`
        );
      ...
      if (targetState == Characteristic.TargetDoorState.OPEN) {
        this.log(
          `GARAGE: ${this.device.name} Current state is closed and target is open. Waiting for sensor change to trigger opening state`
        );
        this.sendGarageDoorCommand(callback);
        return;
      }
    } else if (
      this.currentGarageState == Characteristic.CurrentDoorState.OPENING
    ) {
      if (targetState == Characteristic.TargetDoorState.CLOSED) {
        this.log(
          "GARAGE: " +
            this.device.name +
            " Current state is opening and target is closed. Sending command and changing state to closing"
        );
        this.garageDoorService.setCharacteristic(
          Characteristic.CurrentDoorState,
          Characteristic.CurrentDoorState.CLOSING
      ...
      this.currentGarageState == Characteristic.CurrentDoorState.CLOSING
    ) {
      if (targetState == Characteristic.TargetDoorState.OPEN) {
        this.log(
          "GARAGE: " +
            this.device.name +
            " Current state is closing and target is open. Sending command and setting timeout to complete"
        );
        this.garageDoorService.setCharacteristic(
          Characteristic.CurrentDoorState,
  ...
  completeOpen() {
    if (this.currentGarageState == Characteristic.CurrentDoorState.OPENING) {
      this.log(
        "GARAGE:  " +
          this.device.name +
          "Current door has bee opening long enough, marking open"
      );
      this.garageDoorService.setCharacteristic(
        Characteristic.CurrentDoorState,
      ...
      this.log(
        "GARAGE:  " +
          this.device.name +
          "Opening aborted so not setting opened state automatically"
      );
    }
  }
  // Mirrors change in the state of the underlying isj-js device object.
    ...
    if (this.getSensorState()) {
      if (this.currentGarageState == Characteristic.CurrentDoorState.OPEN) {
        this.log(
          "GARAGE:  " +
            this.device.name +
            "Current state of door is open and now sensor matches. No action to take"
        );
      } else if (
        this.currentGarageState == Characteristic.CurrentDoorState.CLOSED
      ) {
        this.log(
          "GARAGE:  " +
            this.device.name +
            "Current state of door is closed and now sensor says open. Setting state to opening"
        );
        this.garageDoorService.setCharacteristic(
          Characteristic.CurrentDoorState,
          Characteristic.CurrentDoorState.OPENING
        ...
        this.log(
          "GARAGE:  " +
            this.device.name +
            "Current state of door is opening and now sensor matches. No action to take"
        );
      } else if (
        this.currentGarageState == Characteristic.CurrentDoorState.CLOSING
      ) {
        this.log(
          "GARAGE: C " +
            this.device.name +
            "urrent state of door is closing and now sensor matches. No action to take"
        );
      }
    } else {
      if (this.currentGarageState == Characteristic.CurrentDoorState.OPEN) {
        this.log(
          "GARAGE:  " +
            this.device.name +
            "Current state of door is open and now sensor shows closed. Setting current state to closed"
        );
        this.garageDoorService.setCharacteristic(
          Characteristic.CurrentDoorState,
        ...
        this.log(
          "GARAGE:  " +
            this.device.name +
            "Current state of door is closed and now sensor shows closed. No action to take"
        );
      } else if (
        this.currentGarageState == Characteristic.CurrentDoorState.OPENING
      ) {
        this.log(
          "GARAGE:  " +
            this.device.name +
            "Current state of door is opening and now sensor shows closed. Setting current state to closed"
        );
        this.garageDoorService.setCharacteristic(
          Characteristic.CurrentDoorState,
          Characteristic.CurrentDoorState.CLOSED
        ...
        this.log(
          `GARAGE:  ${this.device.name}Current state of door is closing and now sensor shows closed. Setting current state to closed`
        );
        this.garageDoorService.setCharacteristic(
          Characteristic.CurrentDoorState,
          Characteristic.CurrentDoorState.CLOSED
